...
调用 `deepseek-v4-flash`（`app/core/hashing.py` 的 `MODEL`），关思考、稳定前缀、真实 usage。1M 上下文 / 384K 输出上限。成本价**三档**（每 M token）：输入·未命中 ¥1、输入·命中 ¥0.02（前缀缓存命中，≈未命中 1/50）、输出 ¥2——`deepseek.py` 从 usage 的 `prompt_cache_hit/miss_tokens` 拆出命中/未命中。**走平台 key 的翻译按三档成本价 ×1.3（+30% 服务费）扣 credits**（`pricing.py` 的 `cost()` + `SERVICE_FEE_RATE`，平台唯一持续盈利来源；BYOK 客户端直连不经此处、不计费）。**模型能力、参数、前缀缓存、关思考的完整认知见 [`deepseek-v4-flash.md`](deepseek-v4-flash.md)**——调模型 / 改 `deepseek.py` / 动费率前先读它。
...
- **充值 credits**：须注册（余额要跨设备 / 找回）。大陆走 **YunGouOS 微信扫码**（`POST /v1/recharge/create` 选固定档位 ¥10/30/68 下单 → 付款二维码 → 支付后 `notify` 回调验签幂等 grant），**¥1 = ¥1 额度 1:1 入账**（平台盈利在翻译 ×1.3、不在充值加价）。订单号 `rc-{user_id}-{nonce}` 编码 user_id（落在验签字段 outTradeNo 内 → 可信关联账户）。
- **买断 $9.99**：解锁 **BYOK**（自带任意模型 / key、客户端直连），**不送平台额度**——已落地，架构见 [`../front/CLAUDE.md`](../front/CLAUDE.md) 的「BYOK」节。

翻译统一门控（`translate.py`）：owner 余额 > 0 才翻、按 `cost`（×1.3，单位元）实耗扣；无账户或余额 ≤ 0 → 发 `quota`、不翻。**已删 freemium**：原「匿名每天 3 页」+「登录无 credits 的梯度限流」连同 `quota.py`/`tier.py`/`tier_repo.py` 与 `anon_usage`/`quota_tier` 表全部下线。
...
app/
  ...
  routers/ deps.py(current_user_optional) · translate.py(owner 额度门控) · usage.py(余额 + 领赠送 /v1/grant/gift) · auth.py · telemetry.py · admin.py · billing.py(Creem 买断 webhook 收单) · recharge.py(YunGouOS 充值下单 + notify 回调 grant)
...
`users` / `sessions`（账号 + refresh 哈希）· `daily_usage`（每用户每日 token + pages，**登录用户统计用、不再做门控**）· `events` / `error_logs`（打点 / 错误，**只存 host**）· `admins` / `upstream_keys`（管理台）· `credit_txns`（额度流水，**唯一真相**：`owner` 字符串＝`u:{id}` | `d:{deviceId}` 索引、`delta` **`NUMERIC(18,10)` 高精度元**、`kind`、`idempotency_key` 唯一防重复发放；**余额＝该 owner delta 求和**，无 `credit_accounts` 表、不存运行余额，展示层 round 2 位）· `redeem_codes`（买断注册码：`source_ref`=支付订单 id 唯一防重投只签一张）。

> 译文**不在服务端留存**：原跨用户共享缓存已下线（隐私），缓存改为客户端 IndexedDB 本地层，命中本地的块根本不发服务端、不计费。

## API 表面

- `POST /v1/translate`（**SSE，正文专用**：block / done / error / quota；**owner 余额 > 0 才翻、按 ×1.3 扣 credits；无账户或余额 ≤0 发 quota、不翻**；结束发 UsageEvent，登录用户另写 daily_usage；带 `X-Eph-Pub` 头则收发 `ct` 密文）
...
- `POST /v1/recharge/notify`（YunGouOS 异步回调：验签 → 解析 outTradeNo 取 user_id → 幂等 grant credits 到 `u:{id}`；回 `SUCCESS`，验签失败 400 `FAIL`）
...
- `/admin/{login,stats,users,errors,events,keys}`（管理员 JWT `scope:admin`；users 返 `balance`（元）；keys 响应脱敏，绝不回完整 Key）
- `POST /admin/credits/grant`（客服手动调额度：`userId`|`owner` + `amount`（元，正补发 / 负退款纠正）+ 可选 `ref`（幂等键 `admin:{ref}` 防误点双发）；复用 `CreditRepo.grant`，kind=`admin_grant`|`refund`；返 `{owner,amount,balance}`）

## 关键流程

- **翻译**：鉴权识别身份 → 命中客户端本地缓存的块根本不到这里 → **算 owner（登录 `u:{id}`、否则 `d:{deviceId}`）→ 查余额 > 0**（否则发 `quota`、不翻）→ 按 source 去重 → 按 token 预算分批并发调 DeepSeek → 切块 + 标记校验 → `UsageEvent` 时**按 `cost`（×1.3，单位元）实耗扣 owner credits**；登录用户另记 `daily_usage`（统计）。
