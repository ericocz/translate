...
- **赠送 ¥2**：`POST /v1/grant/gift` 按 deviceId 幂等发（`gift:d:{deviceId}` 幂等键，一设备一次），不需注册、绑设备。
- **充值 credits**：须注册（余额要跨设备 / 找回）。
- **买断 $9.99**：解锁 **BYOK**（自带任意模型 / key、客户端直连），**不送平台额度**——第二批，见 [`../front/BYOK-第二批方案.md`](../front/BYOK-第二批方案.md)。
...
app/
  ...
  services/ markers.py · block_splitter.py · deepseek.py(请求体 + SSE + Usage 捕获 + 错误分类)
           ...
           auth.py · creem.py(Creem webhook HMAC 验签 + checkout.completed 解析) · redeem_repo.py(买断注册码幂等签发) · email.py(EmailSender 接口 + LogEmailSender 占位)
...
- `POST /v1/grant/gift`（按 `X-Device-Id` 幂等发赠送 ¥2=2,000,000 micro-¥，一设备一次；缺 deviceId 拒绝）
...
- **BYOK（第二批）**：买断用户自带模型 key，**客户端 SW 直连各 provider**（提示词 / 切块 / token 估算搬到前端），不经本后端、不计费——见方案文档。
