...
- `POST /v1/translate`（**SSE，正文专用**：block / done / error / quota；**owner 任一桶 > 0 才翻、按 active bucket 币种 ×2 扣该桶；无账户或三桶全空发 quota、不翻**；结束发 UsageEvent，登录用户另写 daily_usage；带 `X-Eph-Pub` 头则收发 `ct` 密文）
