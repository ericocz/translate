...
「秒懂翻译 / aha translate」（原名「沉浸式翻译」）—— 一个把网页整页翻成目标语言的浏览器扩展（**源 / 目标语言均由用户选**），正从自用工具演进为可上架产品。本仓库是 **monorepo**：扩展 + 后端 + 管理台。
...
扩展抽取页面可见文本 → 先查**本地 IndexedDB 缓存**（命中不发服务端、不计费）→ 未命中块经 service worker 以 **SSE** 调后端 `/v1/translate` → 后端**去重 / 按 token 预算分批 / 标记校验 / Token 记账**后调 DeepSeek V4 Flash（关思考、稳定前缀）→ 流式回 `{id, 译文}` → 扩展校验标记、重建 DOM、淡入替换、写回本地缓存。

账号（邮箱+密码 JWT）、额度门控（**无匿名 / 免费配额**：靠赠送或充值额度翻译，余额耗尽即停）、Token 记账、付费扣费（credits **多币种分桶**：赠送·人民币 / 充值·人民币 / 充值·美元，扣费优先级 赠送→人民币→美元、按桶币种计价不换汇）、充值收单（大陆 YunGouOS 微信 / 海外 Creem $9.9 美元）、打点 —— **全部在后端**。**已取消买断与 BYOK（2026-06-19）**：Creem 由买断收单改为美元充值收单。服务端**不留存用户译文**（隐私），缓存只在客户端本地。可选**应用层加密**：带 `X-Eph-Pub` 头时原文 / 译文走 `ct` 密文。
...
- **模型**：全程唯一且默认 **DeepSeek V4 Flash**——本仓库未特别说明的「模型」即指它；翻译可选的源 / 目标语言以它支持的为准。
...
- **网络**：作者有 VPN、服务器部署在香港，uv / pnpm 直连官方源即可，**不再用清华镜像 / npmmirror**。后端 httpx 直连 DeepSeek 用 `trust_env=False` 绕开本机 Clash 的 SOCKS 代理；装依赖遇本机代理报错时命令前清 `*PROXY*` 环境变量（`env -u ALL_PROXY -u HTTP_PROXY -u HTTPS_PROXY …`）。
