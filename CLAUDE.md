...
账号（邮箱+密码 JWT）、额度门控（**无匿名 / 免费配额**：靠赠送额度或买断 / 充值配额翻译，余额耗尽即停）、Token 记账、梯度限流、付费扣费（credits）、买断收单、打点 —— **全部在后端**。服务端**不留存用户译文**（隐私），缓存只在客户端本地。可选**应用层加密**：带 `X-Eph-Pub` 头时原文 / 译文走 `ct` 密文。
